...
/* ==============================================================
   ...
   ============================================================== */
async function getAdminDashboard(request, env, ctx) {
  const user = ctx.user;
  ...
  try {
    ...
    const [totalPosts, publishedPosts, totalUsers, postsToday] = await Promise.all([
      ...
      userModel.count(),
      postModel.getPostsCreatedToday()
    ]);
...
/* ==============================================================
   POST MANAGEMENT
   ============================================================== */
async function getPostForm(request, env, ctx) {
  const user = ctx.user;
  const postId = ctx.params?.id || null; // Extract from ctx.params if present
  const logger = new Logger({ context: 'admin' });
  ...
  try {
    ...
    const html = postId 
      ? renderEditPostForm(post, user, config)
      : renderAddPostForm(user, config);
    ...
    logger.error('Error loading post form', { postId, error: error.message });
...
async function updatePost(request, env, ctx) {
  const user = ctx.user;
  const postId = ctx.params.id; // Extract from ctx.params
  const postModel = new PostModel(env.DB);
  const logger = new Logger({ context: 'admin' });
  ...
  try {
    ...
    logger.error('Update post error', { postId, error: error.message });
...
async function getDeleteConfirmation(request, env, ctx) {
  const user = ctx.user;
  const postId = ctx.params.id; // Extract from ctx.params
  const postModel = new PostModel(env.DB);
...
async function deletePost(request, env, ctx) {
  const user = ctx.user;
  const postId = ctx.params.id; // Extract from ctx.params
  const postModel = new PostModel(env.DB);
  const logger = new Logger({ context: 'admin' });
  ...
  try {
    ...
    logger.error('Delete post error', { postId, error: error.message });
...
/* ==============================================================
   SETTINGS MANAGEMENT
   ============================================================== */
async function getSettings(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function updateSettings(request, env, ctx) {
  ...
  try {
    ...
    return Response.redirect(getOrigin(request) + '/admin', 303);
  } catch (error) {
...
/* ==============================================================
   USER MANAGEMENT
   ============================================================== */
async function getUserList(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function getUserForm(request, env, ctx) {
  const user = ctx.user;
  const config = await env.services.config.getConfig();
...
async function createUser(request, env, ctx) {
  ...
  try {
    ...
    return Response.redirect(`${getOrigin(request)}/admin/users`, 303);
  } catch (error) {
...
async function deleteUser(request, env, ctx) {
  const user = ctx.user;
  const userId = ctx.params.id; // Extract from ctx.params
  const userModel = new UserModel(env.DB);
  const logger = new Logger({ context: 'admin' });
  ...
  try {
    ...
    await userModel.delete(userIdInt);
    logger.info('User deleted successfully', { userId: userIdInt });
...
/* ==============================================================
   ...
   ============================================================== */
async function getFederationDashboard(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function syncFederation(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function federatePost(request, env, ctx) {
  const user = ctx.user;
  const postId = ctx.params.id; // Extract from ctx.params
  const logger = new Logger({ context: 'admin' });
...
async function getFederationQueue(request, env, ctx) {
...
/* ==============================================================
   MODERATION
   ============================================================== */
async function getModerationQueue(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function getModerationKeywords(request, env, ctx) {
  try {
...
async function updateModerationKeywords(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
async function checkModeration(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
/* ==============================================================
   QUEUE MANAGEMENT
   ============================================================== */
async function getQueueStatus(request, env, ctx) {
  const user = ctx.user;
...
async function processQueue(request, env, ctx) {
  const user = ctx.user;
...
/* ==============================================================
   EMAIL MANAGEMENT
   ============================================================== */
async function getEmailInjectionForm(request, env, ctx) {
  const user = ctx.user;
...
async function injectEmails(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function fetchEmails(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
async function getPendingReplies(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
async function markReplySent(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
/* ==============================================================
   NOTIFICATIONS
   ============================================================== */
async function sendNotification(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
async function getNotifications(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
/* ==============================================================
   ANALYTICS
   ============================================================== */
async function getAnalytics(request, env, ctx) {
  const user = ctx.user;
  const logger = new Logger({ context: 'admin' });
...
async function debugAnalytics(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
async function checkAnalytics(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
/* ==============================================================
   PROXY MANAGEMENT
   ============================================================== */
async function getProxyStatus(request, env, ctx) {
  const logger = new Logger({ context: 'admin' });
...
export const adminRoutes = {
  ...
  // Proxy Dashboard
  '/admin/proxy': {
    GET: handleProxyRoutes
  },

  // Proxy Tests
  '/admin/proxy/discover-domain': {
    POST: (req, env, ctx) => handleProxyTests.discoverDomain(req, env, ctx)
  },
  '/admin/proxy/test-blog-api': {
    GET: (req, env, ctx) => handleProxyTests.testBlogApi(req, env, ctx)
  },
  '/admin/proxy/test-email-api': {
    GET: (req, env, ctx) => handleProxyTests.testEmailApi(req, env, ctx)
  },
  '/admin/proxy/test-federation': {
    GET: (req, env, ctx) => handleProxyTests.testFederation(req, env, ctx)
  },
  '/admin/proxy/send-test-email': {
    POST: (req, env, ctx) => handleProxyTests.sendTestEmail(req, env, ctx)
  },
  '/admin/proxy/status-stream': {
    GET: (req, env, ctx) => handleProxyTests.statusStream(req, env, ctx)
  },
  '/admin/proxy/status': {
    GET: getProxyStatus
  },
  ...
  '/federation/queue': {
    GET: getFederationQueue
  },

  // Moderation
  '/admin/moderation': {
    GET: getModerationQueue
  },
  '/admin/moderation-keywords': {
    GET: getModerationKeywords,
    POST: updateModerationKeywords
  },
  '/admin/check-moderation': {
    POST: checkModeration
  },

  // Email Management
  '/admin/inject-emails': {
    GET: getEmailInjectionForm,
    POST: injectEmails
  },
  '/admin/fetch-emails': {
    POST: fetchEmails
