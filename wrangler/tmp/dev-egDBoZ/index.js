...
// .wrangler/tmp/bundle-x1Zp90/checked-fetch.js
...
// .wrangler/tmp/bundle-x1Zp90/middleware-loader.entry.ts
init_checked_fetch();

// .wrangler/tmp/bundle-x1Zp90/middleware-insertion-facade.js
init_checked_fetch();
...
// src/utils/jwt.js
init_checked_fetch();
function base64UrlDecode2(base64UrlString) {
  const base64String = base64UrlString.replace(/-/g, "+").replace(/_/g, "/").padEnd(base64UrlString.length + (4 - base64UrlString.length % 4) % 4, "=");
  const binaryString = atob(base64String);
  const len = binaryString.length;
  const bytes = new Uint8Array(len);
...
async function verifyJWT2(token, secret) {
  try {
    ...
    const data = `${encodedHeader}.${encodedPayload}`;
    const signature = base64UrlDecode2(encodedSignature);
    const isValid = await crypto.subtle.verify(
      "HMAC",
      key,
      signature,
    ...
    const decodedPayloadBytes = base64UrlDecode2(encodedPayload);
    const decodedPayloadString = new TextDecoder().decode(decodedPayloadBytes);
    const payload = JSON.parse(decodedPayloadString);
    return payload;
  } catch (error) {
...
var blogRoutes = {
  "/": {
    GET: /* @__PURE__ */ __name(async (request, env) => {
      try {
        const user = await checkAuth2(request, env);
        const { configService: configService2 } = await Promise.resolve().then(() => (init_config2(), config_exports));
        const config2 = await configService2.getConfig(env.DB);
  ...
  "/user/:username": {
    GET: /* @__PURE__ */ __name(async (request, env) => {
      try {
        ...
        const currentUser = await checkAuth2(request, env);
        const { configService: configService2 } = await Promise.resolve().then(() => (init_config2(), config_exports));
        const config2 = await configService2.getConfig(env.DB);
